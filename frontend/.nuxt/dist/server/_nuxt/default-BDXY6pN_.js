...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></nav><div class="px-4 py-4 border-t border-slate-800"><div class="bg-slate-800 rounded-xl p-3 mb-3"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-lg bg-primary-700 flex items-center justify-center shrink-0"><span class="text-primary-200 text-xs font-bold uppercase">${ssrInterpolate(unref(authStore).user?.username?.charAt(0) ?? "?")}</span></div><div class="min-w-0"><p class="text-white text-sm font-medium truncate">${ssrInterpolate(unref(authStore).user?.username)}</p><p class="text-slate-400 text-xs truncate">${ssrInterpolate(unref(authStore).user?.email)}</p></div></div>`);
...
//# sourceMappingURL=default-BDXY6pN_.js.map
